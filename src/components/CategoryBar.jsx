...
function CategoryButton({ className, children, activeCategory, onClick, currentCategory }) {
    ...
    return <button onClick={() => onClick(currentCategory)} className={`${className || ''} md:px-3.5 md:py-1.5 px-2 py-1 whitespace-nowrap rounded-lg w-max transition-all bg-gray-300 ${activeCategory === children && 'bg-black text-white'} hover:bg-gray-500 hover:text-white active:bg-black active:text-white`}>{children}</button>
}
...
export default function CategoryBar() {
    ...
    return (
        <article className={`p-5 relative overflow-x-hidden}`}>
            ...
            }} className="w-32 absolute z-10 left-2 p-3.5 top-1.5  rounded-xl bg-gradient-to-r from-white to-transparent fa-solid fa-chevron-left mt-2.5 cursor-pointer active:text-gray-500"></i>}
